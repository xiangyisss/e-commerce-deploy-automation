...
#Print a message based on a given color.
#Argument: color eg: green, red
...
#Check the service status with a given service, if not active exit script
#Arguments: service name eg: firewalld, mariadb
...
#Check if the firewalld rules are configured, if not exit script
#Arguments: ports eg: 80, 3306
...
#Check if a agiven item is present in the page
#Arguments: Output Item eg: web_page Laptop
function check_item(){
  if [[ $1 = *$2* ]]
  then 
...
#Check MariaDB serive is running
